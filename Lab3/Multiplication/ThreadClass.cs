...
namespace Lab3.Multiplication
{
    internal class ThreadClass
    {
        ...
        private int threadsNumber;              // number of threads
        private Stopwatch[] threadStopwatches;  // time of calculations for each thread
        public double multiplying_time;         // time of calculations for all threads

        public ThreadClass(Matrix m1, Matrix m2, int tn)
        {
            matrix1 = m1;
            matrix2 = m2;
            threadsNumber = tn;
        ...
        public Matrix Multiply()
        {
            ...
            int step = matrix1.Rows / threadsNumber;
            Thread[] threads = new Thread[threadsNumber];
            for (int i = 0; i < threadsNumber; i++)
            {
                int i_temp = i;
                threads[i] = new Thread(() => MultiplyPart(i_temp, step));
            ...
            for (int i = 0; i < threadsNumber; i++)
            {
                // Start stopwatch for each thread
        ...
        private void MultiplyPart(int threadNumber, int step)
        {
            // Get the number of rows for each thread 
            int start = threadNumber * step;
            int end = threadNumber == threadsNumber - 1 ? matrix1.Rows : (threadNumber + 1) * step;
            ...
            threadStopwatches[threadNumber].Stop();
        }
